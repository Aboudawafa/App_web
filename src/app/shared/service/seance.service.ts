...
export class SeanceService {
  ...
  getSeances():Observable<any>{
    return this._http.get("http://localhost:3000/seances");
  }
 getSeancebyId(id : number) :Observable<any> {
    return this._http.get("http://localhost:3000/seances/"+ id );
 }
addSeance(data :Seance) : Observable<any>{
    return this._http.post("http://localhost:3000/seances", data);
}
updateSeance(id : number, data) : Observable<any>{
    return this._http.put("http://localhost:3000/seances/" +id, data);

}
deleteSeance(id : number) : Observable<any>{
    return this._http.delete("http://localhost:3000/seances/"+id );
}
